...
(async function () {
	...
	let getCourseResourceInfo = async function (session, courseInfoId, resourceId, read = input, log = print, retry = defaultRetry) {
		...
		while (!resourceStructure[2][resourceId]) {
			resourceId = await read("The ID of the resource / resource tree you would like to download, or R to return: ");
		...
		log("Fetching resource information. This may take up to a minute for some large course.");
		...
		while (true) {
			...
			while (true) {
				...
				if (choice === "a") {
					...
					if ((await input("Do you want to download manually? (y/N): ")).toLowerCase() === "y") {
